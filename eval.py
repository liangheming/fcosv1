...
@torch.no_grad()
def eval_model(weight_path="weights/fcos_resnet50_last.pth", device="cuda:5"):
    from pycocotools.coco import COCO
    device = torch.device(device)
    with open("config/fcos.yaml", 'r') as rf:
        cfg = yaml.safe_load(rf)
    net = FCOS(**{**cfg['model'], 'pretrained': False, "nms_iou_thresh": 0.6})
